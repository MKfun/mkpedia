...
Раскомментируйте в mk/app.py строки 57, 58 и 59, запустите сервер и перейдите на /migrate (если там вылезет ошибка типа server did not return a valid response - всё ок). Выключите сервер и удалите (забекапив) instance/mkpedik.db, закомментируйте ранее раскомментированные строки (а лучше вообще удалите их) и запустите сервер.
